...
MAX_RESOLUTIONS_PER_RUN = int(
    os.environ.get("BD_POC_MAX_RESOLUTIONS") or 12)
ATTEMPT_TTL_DAYS = 7
...
def run(companies: list[str], desk: str = "comms",
        ...
        fetch=None, resolver=None, profile_resolver=None) -> dict:
    """Fill missing function-owner contacts for `companies` (ranked
    order = spend priority). Returns counters. Never raises."""
    ...
    try:
        ...
        slots = tuple(s for s in _BD_POC_SLOTS.get(
