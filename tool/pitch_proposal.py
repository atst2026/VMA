...
# --------------------------------------------------------------------------
# Company logo resolution.  Tries the company's own domain on a free logo
# service first (good-quality, often transparent), and degrades gracefully to
# a typographic wordmark so the cover never breaks when a logo can't be found
# (or there's no network, e.g. an offline test run).
# --------------------------------------------------------------------------

# High-confidence domains for common UK senior-comms targets. The generic
# candidate-guessing below covers everything else; this just makes the
# marquee names reliable and is cheap to extend.
_KNOWN_DOMAINS: dict[str, str] = {
    "belron": "belron.com",
    "diageo": "diageo.com", "unilever": "unilever.com", "haleon": "haleon.com",
    "reckitt": "reckitt.com", "nestle": "nestle.com", "pepsico": "pepsico.com",
    "tesco": "tesco.com", "sainsbury's": "sainsburys.co.uk",
    "sainsburys": "sainsburys.co.uk", "asda": "asda.com", "morrisons": "morrisons.com",
    ...
    "aviva": "aviva.com", "legal & general": "legalandgeneral.com",
    "schroders": "schroders.com", "monzo": "monzo.com", "revolut": "revolut.com",
    "bp": "bp.com", "shell": "shell.com", "centrica": "centrica.com",
    "national grid": "nationalgrid.com", "severn trent": "severntrent.co.uk",
    "sse": "sse.com", "edf": "edfenergy.com",
    "gsk": "gsk.com", "astrazeneca": "astrazeneca.com",
    "vodafone": "vodafone.com", "bt": "bt.com", "bt group": "bt.com",
    "sky": "sky.com", "rolls-royce": "rolls-royce.com", "bae systems": "baesystems.com",
    "burberry": "burberry.com", "next": "next.co.uk", "currys": "currys.co.uk",
    "heathrow": "heathrow.com", "rio tinto": "riotinto.com", "anglo american": "angloamerican.com",
    "deloitte": "deloitte.com", "ey": "ey.com", "kpmg": "kpmg.com", "pwc": "pwc.com",
    "arup": "arup.com", "experian": "experiancorp.com",
}

_SUFFIX_RX = re.compile(
    r"\b(plc|ltd|limited|llp|holdings|group|the|uk|inc|corporation|corp|company|co)\b",
    re.IGNORECASE)
...
def _domain_candidates(company: str) -> list[str]:
    """Best-effort domains to try, most-likely first. Wrong guesses simply
    404 on the logo service and are skipped."""
    key = (company or "").strip().lower()
    out: list[str] = []
    if key in _KNOWN_DOMAINS:
        out.append(_KNOWN_DOMAINS[key])
    core = _SUFFIX_RX.sub(" ", company or "").strip()
    core_slug = _slug(core) or _slug(company)
    full_slug = _slug(company)
    for base in dict.fromkeys([core_slug, full_slug]):
        if not base:
            continue
        out += [f"{base}.com", f"{base}.co.uk"]
    # de-dupe, keep order
